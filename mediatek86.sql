...
--
-- Hôte : 127.0.0.1:3306
-- Généré le : ven. 06 fév. 2026 à 00:20
...
('10098', '2026-02-05', '10003'),
('10555', '2026-02-01', '10003'),
('10888', '2027-01-23', '10002'),
('10999', '2026-01-25', '10003'),
('17777', '2027-01-23', '10008');
...
--
DROP TRIGGER IF EXISTS `delCommande`;
DELIMITER $$
CREATE TRIGGER `delCommande` BEFORE DELETE ON `commande` FOR EACH ROW BEGIN
...
--
DROP TRIGGER IF EXISTS `ajoutExemplaire`;
DELIMITER $$
CREATE TRIGGER `ajoutExemplaire` AFTER UPDATE ON `commandedocument` FOR EACH ROW BEGIN

   DECLARE i INTEGER DEFAULT 0;
   DECLARE j INTEGER DEFAULT 0;
   DECLARE nouvelleDate DATE;
   
    IF NEW.idSuivi = 3 THEN

    	SELECT ex.numero INTO j FROM exemplaire ex WHERE ex.id = NEW.idLivreDvd ORDER BY ex.numero DESC LIMIT 1;
        SET j = IFNULL(j, 0);

    
     	SELECT c.dateCommande INTO nouvelleDate FROM 		commande c WHERE c.id = NEW.id;
   
   		WHILE i < NEW.nbExemplaire DO
             	SET j = j + 1;
             	INSERT INTO exemplaire (id, numero, dateAchat, photo, idEtat)
             	VALUES (NEW.idLivreDvd, j, nouvelleDate, "", "00001");
             	SET i = i + 1;
   		END WHILE;
	END IF;
END
...
('10002', 'Le Parisien', 'null', 'JN002', '00004', '10014'),
...
('20005', 'OSS 117 : Chinook à Nuuk', '', 'DF001', '00003', '10013'),
('99919', 'Catastrophes au Brésil', '', 'JN002', '00004', '10014'),
('99988', 'testsql\"; DROP TABLE \"livre\";', '', 'DF001', '00001', '10001'),
('99989', 'Catastrophes au Brésil', '', 'JN002', '00004', '10014');
...
('20005', 'Un agent très spécial est chargé d\'envahir le Groenland, mais tout ne va pas se passer comme prévu...', 'Michel Hazanavicius', 103);
...
('00002', 1, '2026-01-22', '', '00001'),
('00002', 2, '2026-01-22', '', '00001'),
('00002', 3, '2026-01-22', '', '00001'),
('00002', 4, '2026-01-22', '', '00001'),
('00002', 5, '2026-01-22', '', '00001'),
('00002', 6, '2026-01-22', '', '00001'),
...
('10011', 515, '2026-01-20', '', '00001'),
('20005', 1, '2026-01-23', '', '00001'),
('20005', 2, '2026-01-23', '', '00001'),
('20005', 3, '2026-01-23', '', '00001'),
('20005', 4, '2026-01-23', '', '00001'),
('20005', 5, '2026-01-23', '', '00001');
...
(1, 'culture', 'f1947f79fdfb8046150959ca09cdd05cb53672ad4c0f49a87bbc7cddf5c91293', 1),
(2, 'prêts', '4c49547cfd8a31f618885a3766bc92e4f0d42b4ce9c317bd38bcac6717dd4759', 2),
(3, 'administratif', 'fa1990549c9ceeb487137cf0e40197431be6d05aa041792e9d8547b109b392eb', 3),
(4, 'admin', '8c6976e5b5410415bde908bd4dee15dfb167a9c873fc4bb8a81f6f2ab448a918', 4);
